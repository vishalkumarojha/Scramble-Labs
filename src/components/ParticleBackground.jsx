...
function ParticleField() {
  ...
  return (
    <Points ref={ref} positions={positions} stride={3} frustumCulled={false}>
      <PointMaterial transparent color="#00D4FF" size={0.015} sizeAttenuation={true} depthWrite={false} opacity={0.4} />
    </Points>
  )
}
      ...
      {connections.map((connection, index) => {
        ...
        return (
          <mesh key={`connection-${index}`} position={center}>
            <cylinderGeometry args={[0.003, 0.003, distance, 8]} />
...
export default function ParticleBackground() {
  return (
    <div className="particle-bg fixed inset-0 z-0 pointer-events-none">
